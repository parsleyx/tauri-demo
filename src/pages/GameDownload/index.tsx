...
export const GameDownloadPage = () => {
    ...
    const downloadFile = useCallback(async (url: string, fileName: string) => {
        ...
        download(url, tmpFile, (res) => {
            ...
            setTotal(res.total);
            const progressText = (res.progress/res.total).toFixed(4) + '%';
    ...
    return <Page>
        <div className="w-1/2 bg-white bg-opacity-80 p-3 rounded-md flex flex-col flex-1 mb-4">
            <div className="flex justify-end">
                <span className="text-lg text-red-500 cursor-pointer" onClick={() => {
                    if(downloadingRef.current) {
                        return;
                    }
                    navigate('/');
            ...
            <div className="flex flex-col items-center">
                <span className="text-sm font-bold py-6">{order?.title}</span>
                <span className="text-sm text-red-500 ">(赠品和游戏已打包...)</span>
                <span className="flex flex-col">
                    <Button disabled={downloading} size="lg" onClick={() => {
                    ...
                    <span className="text-base font-bold text-gray-700 mt-2">备注：如遇到下载问题，请<span className="text-red-500 cursor-pointer" onClick={() => open(customerServiceUrl)}>联系客服</span></span>
                </span>
            </div>
